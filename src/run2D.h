...
void ini_particles(int gl_par_num, std::vector<TPar>& p_arr, 
  ...
  int my_rank;
  MPI_Comm_rank(dm.comm(), &my_rank);
  if (ini_mode == "rand" || ini_mode == "ordered" || ini_mode == "lane") {
    ini_rand(p_arr, gl_par_num, myran, cl, dm);
    if (ini_mode == "ordered") {
      double angle = seed2 / 180. * PI;
      double vx0 = cos(angle);
      double vy0 = sin(angle);
      for (auto& p : p_arr) {
        p.ori = p.ori_next = Vec_2<double>(vx0, vy0);
      }
    } else if (ini_mode == "lane") {
      double angle = PI / 2;
      double vx0 = cos(angle);
      double vy0 = sin(angle);
      for (auto&p : p_arr) {
        int k = int(p.pos.x / seed2);
        double vy = k % 2 == 0? vy0: -vy0;
        p.ori = p.ori_next = Vec_2<double>(vx0, vy);
      }
      if (my_rank == 0) {
        std::cout << "initialize " << gl_par_num << " particles into lanes of width " << seed2 << std::endl;
      }
    }
    t_beg = 0;
  } else {
